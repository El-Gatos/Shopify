...
export async function customerCreate({ firstName, lastName, email, password }) {
  ...
  `;
  const response = await shopifyFetch({ query, variables: { input: { firstName, lastName, email, password, acceptsMarketing: false } } });
...
export async function customerAccessTokenCreate({ email, password }) {
  ...
  `;
  const response = await shopifyFetch({ query, variables: { input: { email, password } } });
...
export async function customerUpdate(accessToken, customer) {
  const query = `
  ...
  `;
  const response = await shopifyFetch({ query, variables: { customerAccessToken: accessToken, customer } });
...
export async function getCustomerOrders(accessToken) {
  ...
  `;
  const response = await shopifyFetch({ query, variables: { customerAccessToken: accessToken } });
  return response.body.data?.customer?.orders?.edges?.map(e => e.node) || [];
}
...
export async function customerRecover(email) {
  ...
  `;
  const response = await shopifyFetch({ query, variables: { email } });
